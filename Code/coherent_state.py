...
class SparseState(CoherentState):
    ...
    def trace_out(self, sites):
        ...
        self.entropy = ff_entropy(self.eigvals)#np.sum(np.log(1 + np.exp(-self.epsl) )) + np.sum(self.epsl/ ( np.exp(self.epsl) + 1 ) )
        #TODO: Set up entropy from epsilon l
        #M = np.transpose(M, c, axis=0)
        #M = np.transpose(M, c, axis=1)

def ff_entropy(eigvals):
    eigvals = np.array([x for x in eigvals if abs(x-1) > 1e-8])
